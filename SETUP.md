...
```bash
cd ~/claude/xiaohongshu-ops/MediaCrawler
uv run python main.py --platform xhs --lt cookie --type search --keywords "作品集辅导"
```

成功后查看 `~/claude/xiaohongshu-ops/MediaCrawler/data/` 是否有输出。

> **注意：** 日常竞品数据已改为手动 Notion 观察方式（2026-03-24），不再依赖 daily_scrape.sh 自动抓取。
