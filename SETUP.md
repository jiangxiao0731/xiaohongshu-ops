...
python3 ~/xiaohongshu-ops/automation/setup_notion.py
```

这一步自动创建所有 database（草稿库、发布记录、竞品周报、花钱日志）和「本周状态」callout。
Database ID 保存到 `~/xiaohongshu-ops/notion-ids.json`。
...
```bash
bash ~/xiaohongshu-ops/automation/daily_scrape.sh
```

成功后查看 `~/xiaohongshu-ops/data/` 是否有 CSV 输出。
...
- [ ] `~/xiaohongshu-ops/notion-ids.json` 文件存在
- [ ] `~/xiaohongshu-ops/MediaCrawler/` 目录存在，`uv run python main.py --help` 正常输出
- [ ] 小红书搜索「作品集辅导」，确认公司号出现
